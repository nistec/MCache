...
using MControl.Cache.Server;
using MControl.Cache.Server.UI;
...
namespace MControl.Caching.Install
{
	...
	public class MainX
	{
        ...
        {
#if DEBUG1
            ServiceManager m_pServer = new ServiceManager();
            m_pServer.Start();
            while (true)
            {
                ...
                if (m_pServer.Loaded)
                {
                    bool ok = MControl.Messaging.RemoteQueue.Get("NC_Quick").CanQueue(100);
                    if (ok)
                    {
