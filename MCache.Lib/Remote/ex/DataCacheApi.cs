...
namespace Nistec.Caching.Remote
{
    ...
    public class DataCacheApi
    {
        ...
        public static DataCacheApi Get(NetProtocol protocol = NetProtocol.Tcp)
        {
         ...
         string RemoteDataCacheHostName;
        bool EnableRemoteException;

        private DataCacheApi()
        {
            RemoteDataCacheHostName = CacheApiSettings.RemoteDataCacheHostName;
        ...
        public void SetValue(string db, string tableName, string column, string filterExpression, object value)
        {
            switch (Protocol)
            {
                case NetProtocol.Http:
                    ...
                    }, RemoteDataCacheHostName, EnableRemoteException);
                    break;
        ...
        public object GetValue(string db, string tableName, string column, string filterExpression)
        {
            ...
            {
                case NetProtocol.Http:
                    return HttpClientCache.SendDuplex<object>(new CacheMessage()
                    {
                        Command = DataCacheCmd.GetDataValue,
                        Key = db,
                        Args = MessageStream.CreateArgs(KnowsArgs.ConnectionKey, db, KnowsArgs.TableName, tableName, KnowsArgs.Column, column, KnowsArgs.Filter, filterExpression)
                ...
                case NetProtocol.Pipe:
                    return PipeClientCache.SendDuplex<object>(new CacheMessage()
                    {
                        Command = DataCacheCmd.GetDataValue,
                        Key = db,
                        Args = MessageStream.CreateArgs(KnowsArgs.ConnectionKey, db, KnowsArgs.TableName, tableName, KnowsArgs.Column, column, KnowsArgs.Filter, filterExpression)
            ...
            return TcpClientCache.SendDuplex<object>(new CacheMessage()
            {
                Command = DataCacheCmd.GetDataValue,
                Key = db,
                Args = MessageStream.CreateArgs(KnowsArgs.ConnectionKey, db, KnowsArgs.TableName, tableName, KnowsArgs.Column, column, KnowsArgs.Filter, filterExpression)
        ...
        public IDictionary GetRow(string db, string tableName, string filterExpression)
        {
            ...
            {
                case NetProtocol.Http:
                    return (IDictionary)HttpClientCache.SendDuplex<IDictionary>(new CacheMessage()
                    {
                ...
                case NetProtocol.Pipe:
                    return (IDictionary)PipeClientCache.SendDuplex<IDictionary>(new CacheMessage()
                    {
                        Command = DataCacheCmd.GetRow,
                        Key = db,
                        Args = MessageStream.CreateArgs(KnowsArgs.TableName, tableName, KnowsArgs.Filter, filterExpression)
            ...
            return (IDictionary)TcpClientCache.SendDuplex<IDictionary>(new CacheMessage()
            {
                Command = DataCacheCmd.GetRow,
                Key = db,
                Args = MessageStream.CreateArgs(KnowsArgs.TableName, tableName, KnowsArgs.Filter, filterExpression)
        ...
        public DataTable GetDataTable(string db, string tableName)
        {
            ...
            {
                case NetProtocol.Http:
                    return (DataTable)HttpClientCache.SendDuplex<DataTable>(new CacheMessage()
                    {
            ...
            return (DataTable)TcpClientCache.SendDuplex<DataTable>(new CacheMessage()
            {
                Command = DataCacheCmd.GetDataTable,
                Key = db,
                Args = MessageStream.CreateArgs(KnowsArgs.TableName, tableName)
        ...
        public DataCacheItem GetItemProperties(string db, string tableName)
        {
            ...
            {
                case NetProtocol.Http:
                    return (DataCacheItem)HttpClientCache.SendDuplex<DataCacheItem>(new CacheMessage()
                    {
            ...
            return (DataCacheItem)TcpClientCache.SendDuplex<DataCacheItem>(new CacheMessage()
            {
                Command = DataCacheCmd.GetItemProperties,
                Key = db,
                Args = MessageStream.CreateArgs(KnowsArgs.TableName, tableName)
