...
namespace Nistec.Caching
{
    ...
    internal class TimerDispatcher : IDisposable
    {
        ...
        private ConcurrentDictionary<string, TimerItem> m_Timer;
        ...
        public TimerDispatcher(int intervalSeconds, int initialCapacity, bool isRemote)
        {
            ...
            m_Timer = new ConcurrentDictionary<string, TimerItem>(concurrencyLevel, initialCapacity);
        ...
        */
        ...
        public void AddOrUpdate(string source, string key, DateTime time, int expiration = 0)
        {
            if (string.IsNullOrEmpty(key))
                return;
            if (expiration == 0)
            ...
            {
                m_Timer[key] = TimerItem.Create(source,DateTime.Now.AddMinutes(expiration), expiration, true);
            }
        }
        public void AddOrUpdate(string source, string key, int expiration, int defaultExpiration=0)
        {
            if (string.IsNullOrEmpty(key))
                return;
        ...
        public bool Remove(string key)
        {
            if (key == null)
                return false;
            TimerItem time;
            if (m_Timer.TryRemove(key, out time))
            {
                CacheLogger.Logger.LogAction(CacheAction.SessionCache, CacheActionState.Debug, "TimerDispatcher Removed : " + key);
                return true;
            }
            return false;
        }
        ...
        public Dictionary<string, TimerItem> Copy()
        {
            ...
            copy = new Dictionary<string, TimerItem>(m_Timer);
        ...
        public string[] GetTimedoutItems()
        {
            ...
            TimeSpan ts = TimeSpan.FromMinutes(1);
            KeyValuePair<string, TimerItem>[] items = m_Timer.ToArray().Where(dic => dic.Value.GetState()==2).ToArray();
    ...
    #region cache timeout
        ...
        */
        ...
        public void Add(string key, int expiration, int defaultExpiration)
        {
            ...
            if (m_Timer.TryGetValue(key, out time))
            {
                time = time.AddMinutes(expiration == 0 ? defaultExpiration : expiration);
                m_Timer[key] = time;
            }
            else
            {
                time = DateTime.Now.AddMinutes(expiration == 0 ? defaultExpiration : expiration);
                m_Timer[key] = time;
            }
            CacheLogger.Logger.LogAction(CacheAction.SessionCache, CacheActionState.Debug, "TimerDispatcher Add : " + key + ", expiration: " + time.ToString());

        }
        public void Update(string key, int expiration, int defaultExpiration)
        {
            if (string.IsNullOrEmpty(key))
                return;
    ...
    #region Report
        ...
        internal static DataTable ReportSchema()
        {
            DataTable dt = new DataTable("SyncBox");
            dt.Columns.Add("ItemName", typeof(string));
            dt.Columns.Add("NextSync", typeof(DateTime));
        ...
        public DataTable TimerReport(string owner)
        {
            DataTable table;
            try
            {
                ...
                foreach (var t in m_Timer)
                {
                    table.Rows.Add(t.Key,t.Value, owner);
                }
            }
            catch (Exception exception)
            {
