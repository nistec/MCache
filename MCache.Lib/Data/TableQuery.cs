   ...
   public class TableInfo
    {
        ...
        public KeyValueArgs Parameters { get; set; }

        public string ToSql()
        {
            return SqlFormatter.GetCommandText(Query, Parameters.Keys);
        }
        public override string ToString()
        {
            return string.Format("{0}&{1}&{2}", ConnectionKey, Query, Parameters == null ? "" : Parameters.ToString());
        }
    }
    ...
    /// </summary>
    public class TableQuery : IDisposable,ISerialEntity
    {

        public static TableQuery Creat(DataTable dt,string mappingName, string [] primaryKey)
        {
            if (dt == null)
            {
        ...
        public static TableQuery CreatWithKey(DataTable dt, string mappingName)
        {
            if (dt == null)
            {
        ...
        public static TableQuery Load(TableQuery source)
        {
            if (source == null)
            {
            ...
            //using (IDbCmd cmd = DbFactory.Create(Owner.ConnectionKey))
            //{
            //    dt = cmd.ExecuteCommand<DataTable>(MappingName);
            //}
            ...
            TableQuery table = CreatWithKey(dt,source.MappingName);
            table.Owner = source.Owner;
            return table;
        }

        public static TableQuery Load(string ConnectionKey, string MappingName,params object[] nameValueParameters)
        {
            if (string.IsNullOrEmpty(MappingName))
            {
                throw new ArgumentException("Invalid MappingName");
            ...
            using (var cmd = DbContext.Create(ConnectionKey, 0, true))
            {
                dt = cmd.QueryDataTable(MappingName, nameValueParameters);
            }
            ...
            TableQuery table = CreatWithKey(dt,MappingName);
            //table.Owner = source.Owner;
            return table;
        }
        ...
        /// </summary>
        public TableQuery(DbSet db, string mappingName) : this()
        {
            Owner = db;
       ...
       /// <param name="disposing"></param>
        ...
        private void OnSizeChanged(GenericEntity currentValue,GenericEntity newValue, bool changeOwner)
        {
            ...
            if(changeOwner)
            {
                Owner.ChanheSizeInternal(newsize - cursize);
            }
            Modified = DateTime.Now;
        ...
        private void OnSizeChanged(long currentSize, long newSize, bool changeOwner)
        {
            ...
            if (changeOwner)
            {
                Owner.ChanheSizeInternal((int)(newSize - currentSize));
            }
            Modified = DateTime.Now;
        ...
        private void OnSizeChanged(GenericEntity ge, int oprator, bool changeOwner)
        {
            ...
            if (changeOwner)
            {
                Owner.ChanheSizeInternal(size * oprator);
            }
            Modified = DateTime.Now;
