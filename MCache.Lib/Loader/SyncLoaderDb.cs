...
namespace Nistec.Caching.Loader
{
    ...
    /// </summary>
    [Serializable]
    public class SyncLoaderDb //: IDataCache
    {

        public SyncLoaderDb Copy()
        {
                        
            //var syncCopy = this._SyncTables.Copy();
            //syncCopy.Owner = this;

            return new SyncLoaderDb()
            {
        ...
        public SyncLoaderDb(string connectionKey)
        {
        ...
        /// </summary>
        /// <param name="connectionKey"></param>
        public SyncLoaderDb(string connectionKey, SyncOption syncOption,bool enableTrigger)
        {
            SyncState = CacheSyncState.Idle;
       ...
       /// <param name="disposing"></param>
        ...
        public void Start(int intervalSeconds)
        {
            ...
            if (_SyncOption == SyncOption.Auto)
            {
                ...
                DataLoaderEntity[] items = _SyncTables.GetItems();
                if (items != null && items.Length > 0)
                {
                    foreach (DataLoaderEntity source in items)
                    {
        ...
        public void Stop()
        {
            ...
            if (_SyncOption == SyncOption.Auto)
            {
                ...
                foreach (DataLoaderEntity source in _SyncTables.GetItems())
                {
                    //evt-
                    source.SyncSourceChanged -= new SyncDataSourceChangedEventHandler(source_SyncSourceChanged);
                }
