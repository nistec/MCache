...
namespace Nistec.Caching.Loader
{
    ...
    /// </summary>
    [Serializable]
    public class SyncLoaderStream : SyncLoaderBase<EntityStream>//, ISyncLoaderStream 
    {
 
        #region ctor

        /// <summary>
        /// Create new <see cref="SyncLoaderStream"/>
        /// </summary>
        public SyncLoaderStream(bool isAsync)
            : base(isAsync)
        {
        ...
        public SyncLoaderStream(string connectionKey, string entityName, string mappingName, string[] keys,string columns, SyncTimer timer, bool enableNoLock, int commandTimeout, bool isAsync)
            : base(isAsync)
        {
        ...
        /// <param name="commandTimeout"></param>
        /// <param name="isAsync"></param>
        public SyncLoaderStream(string connectionKey, string entityName, string mappingName, string[] sourceName, EntitySourceType sourceType, string[] keys, string columns, SyncTimer timer, bool enableNoLock, int commandTimeout, bool isAsync)
            : base(isAsync)
        {
            Set(connectionKey, entityName, mappingName, sourceName, sourceType, keys, columns, timer, enableNoLock, commandTimeout);
        }

        internal SyncLoaderStream(IKeyValue dic, bool isAsync)
            : base(isAsync)
        {
        ...
        /// <param name="isAsync"></param>
        public SyncLoaderStream(SyncEntity entity, bool isAsync)
            : base(entity,isAsync)
        {
       ...
       /// <returns></returns>
        ...
        public GenericKeyValue GetEntityItems(bool copyBody)
        {
            ...
            {
                if (copyBody)
                    kv.Add(item.Key, item.Copy());
                else
                    kv.Add(item.Key, item.Clone());
            }
            return kv;
        }
        ...
        public EntityStream GetEntityStream(ComplexKey info)
        {
            ...
            var es= GetEntityStreamInternal(info.ItemKey);
            if (es == null)
                return null;
            return es.Copy();
        }
        ...
        internal EntityStream GetEntityStreamInternal(string key)
        {
            if (!IsReady)
            {
                CacheLogger.Info("SyncLoaderStream is not ready");
                return null;// throw new Exception("SyncLoaderStream is not ready");
            }
        ...
        public NetStream GetItemStream(ComplexKey info)
        {
            if (info == null)
                return null;
            return GetItemStream(info.ItemKey);
        }
        ...
        public byte[] GetItemBinary(ComplexKey info)
        {
            if (info == null || info.IsEmpty)
                return null;
            return GetItemBinary(info.ItemKey);
        }
        ...
        {
            Console.WriteLine("Debuger-SyncLoaderStream.Refresh...");
